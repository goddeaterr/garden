...
export function scheduleStartupScrape() {
  ...
  setTimeout(async () => {
    try {
      // Determine the local server URL - try common ports
      ...
      console.log(`[Scraper] Auto-scrape complete: ${data.liveAdded ?? 0} live products added, ${data.downloadedImages ?? 0} images downloaded, ${data.totalProducts ?? 0} total products, ${data.marketplace ?? 0} groups`);
    } catch (e: any) {
