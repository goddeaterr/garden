...
export default function AdminPage() {
  ...
  const handleScrapeAll = async () => {
    if (!confirm('This will scrape all 6 nursery sources. It may take 2-3 minutes. Continue?')) return;
    setScraping(true); setScrapeResult(null); setError('');
    try {
      const r = await fetch('/api/admin/scrape', { method:'POST', headers: ah(), body: JSON.stringify({ action:'scrape_all' }) });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error||'Scrape failed');
      setScrapeResult({ found: d.totalAdded || 0, added: d.totalAdded || 0 });
      setSuccess(`Full scrape complete. Added ${d.totalAdded} new products. Marketplace rebuilt.`);
      await fetchScraped();
    } catch (e: any) { setError(e.message); } finally { setScraping(false); }
  };
              ...
              <div className="lg:col-span-2 bg-forest-900 rounded-2xl border border-forest-800 p-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    <span className="text-[10px] text-forest-600">or</span>
                    <div className="flex-1 h-px bg-forest-800"/>
                  </div>
                  <button onClick={handleScrapeAll} disabled={scraping}
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-amber-700 hover:bg-amber-600 text-white font-semibold text-[13px] disabled:opacity-50 transition-colors">
                    {scraping ? <Loader2 size={15} className="animate-spin"/> : <Zap size={15}/>}
                    {scraping ? 'Scraping all sources…' : '⚡ Scrape ALL nurseries at once'}
                  </button>
                  {scrapeResult && (
