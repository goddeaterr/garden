...
export function MarketplaceTeaser() {
  ...
  return (
    <section className="botanical-section-texture relative overflow-hidden py-20 px-4 sm:px-6 bg-gradient-to-b from-white to-forest-50 dark:from-forest-900 dark:to-forest-950">
        ...
        <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
          <div>
            <div className="text-[11px] tracking-[0.3em] uppercase text-forest-600 dark:text-forest-400 mb-3 flex items-center gap-2">
              <TrendingDown size={12}/>{m.teaserEyebrow}
            </div>
            ...
            <p className="text-[15px] text-forest-600 dark:text-forest-400 mt-3 max-w-lg">
              {m.teaserSub
                .replace('{products}', String(stats.products || '…'))
                .replace('{sellers}', String(stats.sellers || '…'))}
            </p>
          </div>
          <Link href="/marketplace"
            ...
            {m.seeAllOffers} <ArrowUpRight size={15} className="group-hover:rotate-45 transition-transform"/>
          </Link>
        </div>
            ...
            {groups.map((group, i) => {
              ...
              return (
                <motion.div key={group.slug}
                  ...
                  className="market-card group relative bg-white dark:bg-forest-900 rounded-2xl border border-forest-200/60 dark:border-forest-800/50 p-4 hover:border-forest-300 dark:hover:border-forest-700 hover:shadow-md transition-all cursor-pointer"
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="text-right">
                      <div className="text-[11px] text-forest-500">{m.from}</div>
                  ...
                  <div className="mt-3 pt-3 border-t border-forest-100 dark:border-forest-800 flex items-center justify-between">
                    <span className="text-[11px] text-forest-500 flex items-center gap-1"><Users size={10}/>{group.offers.length} {m.sellersLabel}</span>
                    {savings > 1 && <span className="text-[11px] font-semibold text-emerald-600">{m.saveUpTo}{savings.toFixed(0)}</span>}
        ...
        {!loading && groups.length > 0 && (
          <div className="text-center mt-8">
            <Link href="/marketplace" className="text-[13px] text-forest-600 dark:text-forest-400 hover:text-forest-900 dark:hover:text-white underline underline-offset-4 transition-colors">
              {m.viewAllCount.replace('{count}', String(stats.products))}
            </Link>
          </div>
        )}
      </div>
