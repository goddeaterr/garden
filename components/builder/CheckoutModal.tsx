...
export function CheckoutModal() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = [
        'Garden Plan Order Request',
        '',
        'CUSTOMER DETAILS',
        `Name: ${sanitize(form.firstName)} ${sanitize(form.lastName)}`,
        `Email: ${form.email}`,
        `Phone: ${form.phone}`,
        form.deliveryOption === 'delivery'
          ? `Address: ${sanitize(form.address)}${form.city ? ', ' + sanitize(form.city) : ''}`
          : 'Delivery: Pickup from nursery',
        '',
        'TREE ORDER',
        treeList,
        '',
      ...
      const subject = encodeURIComponent(`Garden Plan Order — ${sanitize(form.firstName)} ${sanitize(form.lastName)} — ${formatPrice(total)}`);
