...
import { FileText, ShoppingBag, ShoppingCart, X, Trash2, Loader2, ChevronUp } from 'lucide-react';
              ...
              {items.length > 0 && (
                <div className="p-3 space-y-2 flex-shrink-0">
                  ...
                  <button onClick={handleRequestQuote} disabled={quoting}
                    className="w-full flex items-center justify-center gap-1.5 py-3 rounded-xl bg-emerald-600 text-white text-[13px] font-bold shadow-lg shadow-emerald-900/20 disabled:opacity-60">
                    {quoting ? <Loader2 size={13} className="animate-spin" /> : <ShoppingBag size={13} />}
                    {quoting ? s.preparing : s.requestQuote}
                  </button>
                  {/* Secondary CTA */}
                  <a href="#catalog" onClick={() => setSheetOpen(false)}
                    className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-forest-900 dark:bg-forest-100 text-white dark:text-forest-900 text-[13px] font-bold">
