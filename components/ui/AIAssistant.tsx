...
const SUGGESTIONS = [
  "I need a hedge for privacy, windy coastal area",
  "3 low-maintenance fruit trees for a small garden",
  "Best trees for Lithuanian winters",
  "Fast-growing trees for shade?",
];

const WELCOME = `Hello! 🌳 I'm your garden advisor at MB Plant House. Tell me about your garden — size, location, sun exposure — and I'll recommend the perfect trees for you.

*Sveiki! Galite rašyti lietuviškai ar rusiškai.*`;
  ...
  return (
    <>
      ...
      <motion.button
        ...
        aria-label="Open garden advisor"
      >
          ...
          >
            ...
            <div className="flex-shrink-0 flex items-center justify-between px-4 py-3.5" style={{ background: 'linear-gradient(135deg,#1e3d16,#3b6d11)' }}>
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-white font-bold text-[13px] leading-tight">Garden Advisor</div>
                  <div className="text-white/50 text-[10px]">MB Plant House · AI powered</div>
                </div>
              </div>
              <div className="flex gap-1.5">
                <button onClick={reset} title="New conversation" className="w-7 h-7 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white/60 hover:text-white transition-colors">
                  <RotateCcw size={12}/>
                </button>
            ...
            <div className="flex-shrink-0 px-3 pb-3 pt-2 border-t border-forest-100">
              <div className="flex items-end gap-2 bg-forest-50 rounded-2xl border border-forest-200 px-3 py-2 focus-within:border-forest-400 transition-colors">
                <textarea ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey}
                  placeholder="Ask about trees, planting, care…" rows={1} maxLength={500}
              ...
              <p className="text-[10px] text-forest-400 text-center mt-1">Enter to send · Shift+Enter for new line</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
